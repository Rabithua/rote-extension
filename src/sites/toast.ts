...
export class CaptureToast {
  ...
  show(key: string, recovery = false) {
    ...
    if (!this.host) {
      ...
      .message{display:flex;align-items:center;gap:12px;padding:16px;border-radius:10px;border:1px solid oklch(.922 0 0);background:oklch(1 0 0);box-shadow:0 4px 12px #0000001a}
      ...
      button{font:inherit;color:inherit;border:0;background:transparent;cursor:pointer;flex:none;padding:0}button[hidden]{display:none}.action{text-decoration:underline;text-underline-offset:3px}.close{display:flex;align-items:center;justify-content:center;padding:4px;border-radius:4px}.close:hover{background:#8882}button:focus-visible{outline:2px solid currentColor;outline-offset:3px}
      @keyframes spin{to{transform:rotate(360deg)}}@media(prefers-reduced-motion:reduce){.saving{animation:none}}@media(prefers-color-scheme:dark){:host{color:oklch(.985 0 0)}.message{background:oklch(.205 0 0);border-color:#ffffff1a}}
      @media(max-width:480px){.message{flex-wrap:wrap}.text{flex:1}.action{margin-left:28px}.close{margin-left:auto}}`;
  ...
  hide() {
    if (this.timer) clearTimeout(this.timer);
    this.timer = undefined; this.host?.remove(); this.host = undefined; this.dismissed = true;
  }
}
