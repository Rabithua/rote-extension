...
export function ConnectionForm({ settings, onSaved, t }: { settings: Settings | null; onSaved: (settings: Settings) => void; t: (key: string) => string }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const result = await send({ type: 'settings:save', settings: { apiUrl: normalized, openKey, language, theme, defaultTags: parseDefaultTags(defaultTags), defaultVisibility } });
