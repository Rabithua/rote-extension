...
更新时替换原文件夹内容，然后在扩展管理页点击重新加载，并刷新 X 页面；关闭升级前已打开的设置页，再点击工具栏 Rote 图标重新打开。不要删除扩展再安装，否则浏览器会清除本地配置和待恢复任务。
...
「自动添加平台标签」默认关闭。开启后，采集 X 帖子会添加 `X` 标签，与默认标签去重；含平台标签在内最多 20 个标签。此开关只影响新采集任务。
