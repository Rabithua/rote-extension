...
type Note = {id:string;content:string;state:string;attachments:Attachment[]};
const notes: Note[] = [];
...
const server=createServer(async (req,res)=>{
  ...
  if(path==='/notes' && req.method==='POST'){
    ...
    const note={id:randomUUID(),content:body.content,state:body.state,attachments:[]};notes.push(note);
