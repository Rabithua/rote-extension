...
export default defineContentScript({
  ...
  runAt: 'document_idle',
  main(ctx) {
    const bridge = createAdapterBridge('x');
    const adapter = new XAdapter(bridge);
    const listener = (message: { type?: string; task?: TaskView }, sender: chrome.runtime.MessageSender) => {
      ...
      if (sender.id === chrome.runtime.id && message.type === 'task:changed' && message.task && bridge.accept(message.task)) adapter.update(message.task);
    };
